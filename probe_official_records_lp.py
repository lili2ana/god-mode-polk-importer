...
def parse_result_rows(body):
    ...
    for m in row_re.finditer(text):
        ...
        rows.append({'name':name,'from_party':from_party,'date':m.group('date'),'type':m.group('type').upper(),'book':m.group('book'),'page':m.group('page'),'legal':legal,'file_no':m.group('file_no'),'status':m.group('status').upper(),'flag':(m.group('flag') or '').upper(),'parcel_id':parcel_id})
    return rows
...
def rows_from_json(obj):
    ...
    def walk(x):
        if isinstance(x, dict):
            ...
            if re.search(r'\b\d{10}\b', blob) and re.search(r'\b(?:LP|L PEN)\b', blob, re.I):
                hits.append(x)
        ...
        with sync_playwright() as p:
            ...
            def on_response(resp):
                try:
                    ...
                    item={'url':resp.url,'status':resp.status,'method':req.method,'resource_type':req.resource_type,'content_type':ctype}
                    network.append(item)
                    if 'json' in ctype or req.resource_type in ('xhr','fetch'):
                        try:
                            txt=resp.text()
                            item['body_excerpt']=txt[:3000]
                            if 'json' in ctype:
                                try:
                                    obj=json.loads(txt)
                                    hits=rows_from_json(obj)
                                    if hits:
                                        candidate_payloads.append({'url':resp.url,'status':resp.status,'hit_count':len(hits),'sample':hits[:5]})
                                except Exception:
                                    pass
                        except Exception:
                            pass
            ...
            if matched:
                report['status']='blocked'; report['block_indicators']=matched
            ...
            for i in range(visible_inputs.count()):
                ...
                if ('document type' in ph) and ('search document' not in ph) and doc_input is None: doc_input=el
                if 'mm/dd/yyyy' in ph: date_inputs.append(el)
            if len(date_inputs)>=2: from_input,to_input=date_inputs[0],date_inputs[1]
            ...
            if not (doc_input and from_input and to_input):
                raise RuntimeError('Could not identify search inputs')
            ...
            for i in range(searches.count()):
                b=searches.nth(i)
                if b.is_visible(): b.click(timeout=5000); clicked=True; break
            ...
            # Harvest virtualized rows by progressively scrolling every scrollable container.
            harvested={}
            stagnant=0
            for step in range(80):
                cur=page.locator('body').inner_text(timeout=5000)
                for r in parse_result_rows(cur):
                    harvested[(r['name'],r['file_no'])]=r
                before=len(harvested)
                page.evaluate("""() => {
                  const els=[document.scrollingElement,...Array.from(document.querySelectorAll('*')).filter(e=>e.scrollHeight>e.clientHeight+20)];
                  for (const e of els) { try { e.scrollTop = Math.min(e.scrollHeight, e.scrollTop + Math.max(400, e.clientHeight*0.9)); } catch(_){} }
                }""")
                page.wait_for_timeout(150)
                cur2=page.locator('body').inner_text(timeout=5000)
                for r in parse_result_rows(cur2): harvested[(r['name'],r['file_no'])]=r
                if len(harvested)==before: stagnant+=1
                else: stagnant=0
                if report.get('total_party_rows') and len(harvested)>=report['total_party_rows']: break
                if stagnant>=12: break
            rows=list(harvested.values())
            report['parsed_party_rows']=rows
            report['parsed_party_row_count']=len(rows)
            report['network_requests']=network[-200:]
            report['candidate_json_payloads']=candidate_payloads
            report['candidate_json_payload_count']=len(candidate_payloads)

            unique_documents={}
            for row in rows: unique_documents.setdefault(row['file_no'],row)
            ...
            log('STAGE: harvested rows =', len(rows), 'json candidates =', len(candidate_payloads))

            page.screenshot(path=str(SHOT),full_page=True)
            if report.get('total_party_rows') is not None and len(rows)!=report['total_party_rows'] and not candidate_payloads:
                raise RuntimeError(f'Full-result retrieval mismatch: harvested={len(rows)} reported={report["total_party_rows"]}; no JSON/XHR payload found')
