...
function fixture(
  ...
  const handler = buildWeb({
    ...
    microsoft: options.disabled ? undefined : async (target) => {
      calls.start++;
      callback = target;
      ...
      return {
        url: options.unsafeRedirect ? "https://attacker.invalid/" : url.href,
        dispose: () => {
          calls.dispose++;
        },
...
Deno.test("Microsoft: callback binds browser flow, exchanges once, and issues only opaque Strict session", async () => {
  const f = fixture();
  ...
  const privatePage = await f.handler(f.req("/", "GET", session.split(";")[0]));
  eq((await privatePage.text()).includes("Your property workspace"), true);
  await f.handler(f.req(flow.callback + "&code=" + code, "GET", flow.cookie));
  eq(f.calls.exchange, 1);
  eq(f.calls.dispose, 1);
  eq(f.calls.email, 0);
});
Deno.test("Microsoft: expiry while exchange or authorization is pending cannot issue a session", async () => {
  for (const stage of ["exchange", "read"]) {
    ...
    eq(
      result.headers.getSetCookie().some((x) => x.startsWith("godmode_local=")),
      false,
    );
  }
});
Deno.test("Microsoft SDK: unconfirmed, anonymous and failed confirmed-user lookups fail closed", async () => {
  for (const state of ["unconfirmed", "anonymous", "error"]) {
    ...
    const flow = await microsoftLogin("sb_publishable_fixture", request)(
...
Deno.test("Microsoft: disabled and cross-origin starts never initiate provider or email", async () => {
...
Deno.test("Microsoft: callback without browser cookie or with another browser flow cannot exchange", async () => {
  const f = fixture();
...
Deno.test("Microsoft: expired flow and logout cancel pending exchanges", async () => {
...
Deno.test("Microsoft: duplicate codes, arbitrary next URL, errors and oversized codes are stripped without exchange", async () => {
  for (
    const suffix of [
      "&code=" + code + "&code=" + code,
...
Deno.test("Microsoft: wrong verified UUID and gateway denial never authorize a workspace", async () => {
...
Deno.test("Microsoft: in-flight callback cannot complete after logout or race a duplicate exchange", async () => {
  let release!: () => void;
...
Deno.test("Microsoft: unsafe provider redirects are rejected and new starts are throttled", async () => {
...
Deno.test("Microsoft SDK: isolated S256 verifiers and confirmed-user lookup with no provider token retention", async () => {
  ...
  const begin = microsoftLogin("sb_publishable_fixture", request);
  ...
  for (const [id, flow] of [["one", one], ["two", two]] as const) {
    ...
    eq(url.searchParams.get("provider"), "azure");
    eq(url.searchParams.get("scopes"), "email");
