...
CHECK (scope IN ('god-mode-dd-worker','god-mode-dd-finalize','god-mode-crm-feed','god-mode-title-access-worker'));
DO $setup$
DECLARE token text;
BEGIN
 token := encode(extensions.gen_random_bytes(32),'hex');
 PERFORM vault.create_secret(token,'internal_god_mode_title_access_worker','Scoped title/access worker authentication');
 INSERT INTO god_mode_ops.internal_function_credentials(scope,token_digest)
 VALUES ('god-mode-title-access-worker',encode(extensions.digest(token,'sha256'),'hex'));
END $setup$;

-- Preserve existing schedules. Title/access is an authenticated manual worker.
...
BEGIN
 IF p_scope IS NULL OR p_scope NOT IN ('god-mode-dd-worker','god-mode-dd-finalize','god-mode-crm-feed','god-mode-title-access-worker')
